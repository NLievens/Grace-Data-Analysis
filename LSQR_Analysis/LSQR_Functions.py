...
def EWH_grid(SH_arr, cov_SH_arr, year_lst, sample_time, lat_precis=30, lon_precis=60, lat_range=(-np.pi/2, np.pi/2), lon_range=(-np.pi, np.pi), max_order=96, J2=False, calc_uncertainty=False, excel_output=True, file_name='output.xlsx'):
    ...
    print("\n=== True Values ===\n")
    print(f"EWH max  ➜  {np.max(earth_grid_EWH):.4f} [mm]")
    print(f"EWH min  ➜  {np.min(earth_grid_EWH):.4f} [mm]")
    print(f"EWH mean ➜  {np.mean(earth_grid_EWH):.4f} [mm]")
    print(f"EWH std  ➜  {np.std(earth_grid_EWH):.4f} [mm]")
...
'''
TODO
- Change the delta values for C and S from (value - first value) to (value - average value) (Check ChatGPT)
'''



# Define Least Squares Regression
def LSQR_COEF_TEST(data_year_arr, date_year_arr, year_lst, max_order=96):
    """
    Fits linear regression models to spherical harmonic coefficients over a specified year range.
    ...
    # Define Delta Values
    delta_CS_vector_fil = data_CS_vector_lst_fil
    delta_CS_std_vector_fil = data_CS_std_vector_lst
...
def EWH_GRID_TEST(SH_arr, cov_SH_arr, year_lst, sample_time, lat_precis=30, lon_precis=60, lat_range=(-np.pi/2, np.pi/2), lon_range=(-np.pi, np.pi), max_order=96, J2=False, calc_uncertainty=False, excel_output=True, file_name='output.xlsx'):
    ...
    # Example: Evaluate model for all months
    C_fit_all = []
    S_fit_all = []

    for t in all_month_times:  # in days from t0
        basis_val = T_row(t, year_lst)  # seasonal + noise terms, no polynomial
        C_fit_all.append(basis_val @ SH_arr[:, :, :, 0])  # vector multiply
        S_fit_all.append(basis_val @ SH_arr[:, :, :, 1])

    C_fit_all = np.array(C_fit_all)  # shape: [months, num_coeffs]
    S_fit_all = np.array(S_fit_all)
